...
<task type="auto">
  <name>Task 1: Implement extractCitations pipeline</name>
...
export async function extractCitationsAsync(
  ...
  const tokens = await tokenize(cleaned, allPatterns)
...
export function extractCitations(
  ...
  // Sync wrapper: same logic but without async tokenize
  // For MVP, tokenize is async but doesn't actually need to be
  // So we can make sync version by using Promise.resolve or refactoring tokenize
  // For now, sync version throws if called (forcing async usage)
  throw new Error('Use extractCitationsAsync for now - sync version coming in Phase 3')
}
```
...
Check that extractCitationsAsync is exported
Verify pipeline compiles and type-checks correctly
  </verify>
  <done>extractCitationsAsync() pipeline orchestrates clean → tokenize → extract layers</done>
</task>
...
This provides:
1. **Convenience API:** extractCitationsAsync(text) for most users
...
Verify TypeScript can import from package: `import { extractCitationsAsync } from 'eyecite-ts'`
  </verify>
...
**Test 4: Extract with position accuracy**
...
Use `await extractCitationsAsync(input)` for all tests. Add descriptive test names.
...
Check that position accuracy test passes (originalSpan correctness)
  </verify>
  <done>Integration tests validate full pipeline on legal text, including position accuracy and multiple citation types</done>
</task>
...
3. Check that extractCitationsAsync is exported from src/index.ts
...
6. Verify position accuracy: original.substring(span.original) matches citation text
</verification>

<success_criteria>
- Developer can call extractCitationsAsync(text) and receive all citations
- Pipeline executes clean → tokenize → extract → translate in sequence
- Public API exports convenience and granular functions
- Integration tests validate on realistic legal text samples
- Position tracking is accurate (originalSpan correctness verified)
