...
**Timeout protection:** Wrap matchAll() in try-catch. If pattern throws or takes >50ms (use setTimeout + Promise.race), skip pattern and log warning to console. Continue with remaining patterns.

**Note:** This is async to support future timeout enforcement with Promise.race. For MVP, simple try-catch is sufficient.
...
- Input: "See 42 U.S.C. § 1983 for details"
- Expected: Token with text matching "42 U.S.C. § 1983", type "statute"
- Assert: Token found at correct position

**Test 3: Tokenize multiple citations in text**
- Input: "In Smith v. Doe, 500 F.2d 123, the court cited 42 U.S.C. § 1983"
...
Use `await tokenize(input, allPatterns)` syntax. Import all pattern arrays for testing.
