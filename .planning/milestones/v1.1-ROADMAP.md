...
Improve citation extraction accuracy with full spans, party names, parallel citation linking, blank page support, and complex parentheticals. All new fields optional for backward compatibility.
...
**Key Decisions:**
- All v1.1 fields optional for 100% backward compatibility with v1.0
- Unified parenthetical parser replacing fragmented year/court extraction
- Backward search up to 150 chars for case name extraction
- Depth tracking for chained parentheticals (en banc, per curiam)
- Defendant-first storage in supra resolution history (Bluebook convention)
- Comma-only separator for parallel citations (Bluebook standard)
- groupId format: ${volume}-${reporter}-${page} (deterministic, human-readable)
- Key field matching strategy for golden corpus tests (vs full snapshots)

**Issues Resolved:**
- Cross-citation case name matching (digit-period-space boundary detection)
- Signal word stripping consistency (moved into extractPartyNames)
- Dash placeholder matching (lookahead assertion instead of \b word boundary)
- stripDateFromCourt extended for full month names and numeric dates

**Issues Deferred:**
- Statute short-form pipeline (v1.2)
- Custom reporter API (v1.2)
- International citation formats (v2.0)
