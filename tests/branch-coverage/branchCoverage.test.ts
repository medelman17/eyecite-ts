...
import { getReportersSync, loadReporters } from "@/data/reporters"
import {
  extractCase,
...
import { extractId, extractShortFormCase, extractSupra } from "@/extract/extractShortForms"
...
/**
 * Add threaded named groups to a manually constructed token. Preserves the
 * original span so position-translation tests keep their existing assertions.
 */
...
describe("tokenizer.ts — catch block for throwing patterns", () => {
  ...
  it("should handle non-Error thrown values", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("string-thrower"), "string error")
...
describe("extractCase.ts — throw branch", () => {
  it("should throw on a token with no threaded groups (decline path)", () => {
    // A token with no groups (e.g., produced before group threading) causes
    ...
    expect(() => extractCase(token, createIdentityMap())).toThrow("Missing required capture group")
  })
})
...
describe("extractShortForms.ts — throw branches and position fallbacks", () => {
  it("extractId should throw on text that does not match Id regex", () => {
    ...
    expect(() => extractId(token, createIdentityMap())).toThrow("Failed to parse Id. citation")
  })
