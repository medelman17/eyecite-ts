...
describe("Golden Corpus Extraction Accuracy", () => {
  ...
  describe("Performance Benchmarks", () => {
    it("extracts citations from 10KB document in <100ms", () => {
      ...
      const perfSample = goldenCorpus.samples.find((s) => s.id === "performance-10kb-document")
      ...
      expect(citations.length).toBeGreaterThanOrEqual(perfSample.performanceBenchmark!.minCitations)
  ...
  describe("Quality Targets", () => {
    ...
    it("party names are extracted for adversarial cases", () => {
      const adversarialSamples = goldenCorpus.samples.filter((s) => s.id.startsWith("case-"))
