...
const allPatterns = [...casePatterns, ...statutePatterns, ...journalPatterns, ...neutralPatterns]
...
describe("tokenize", () => {
  it("should tokenize case citation", () => {
    ...
    expect(input.substring(federalToken?.span.cleanStart, federalToken?.span.cleanEnd)).toBe(
      "500 F.2d 123",
    )
  })
  ...
  it("should tokenize statute citation", () => {
    ...
    expect(input.substring(tokens[0].span.cleanStart, tokens[0].span.cleanEnd)).toBe(
      "42 U.S.C. § 1983",
    )
  })
  ...
  it("should tokenize multiple citations in text", () => {
    ...
    for (let i = 1; i < tokens.length; i++) {
      expect(tokens[i].span.cleanStart).toBeGreaterThanOrEqual(tokens[i - 1].span.cleanStart)
    }
  })
