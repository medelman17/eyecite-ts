...
2. **Floor is Node ≥ 22; duplicate named groups are used.** PR 1 bumps `engines.node` to `>=22` and the CI matrix to **22 / 24 / 26** — a deliberate, breaking runtime-floor change with its own changeset. This unlocks ES2025 *duplicate named capturing groups* (V8 12.4 / Node 22+), so the same concept is named identically across alternation branches: space-form and comma-form both use `(?<page>…)`, and the extractor reads `token.groups.page` directly (the non-participating branch's group is simply absent). **No per-branch coalescing.** *Rejected:* staying on `>=18` with distinct branch names + coalescing — the maintainer chose the cleaner floor bump over extractor bookkeeping.
