...
2. **Floor is Node ≥ 22; distinct per-branch group names + coalesce (NOT duplicate named groups).** PR 1 bumps `engines.node` to `>=22` and the CI matrix to **22 / 24 / 26** — a deliberate, breaking runtime-floor change with its own changeset. *Original v2 plan:* use ES2025 *duplicate named capturing groups* (`(?<page>…)` on both alternation branches) and read `token.groups.page` directly. **This was empirically falsified during the caseCore migration (#888):** CI's Node 22/24/26 (via `setup-node`) throws `SyntaxError: Duplicate capture group name` at module load — 262 suites failed under coverage — even though local Node 24.x *accepts* them (a local-passes / CI-fails trap; the panel's DRIFT-1, vindicated). **Corrected approach:** when one terminal appears in two alternation branches, give the branches **distinct names** (`page` / `pageComma`) and **coalesce in the extractor** (`optionalGroup("page") ?? optionalGroup("pageComma")`). This works on every Node, so the `>=22` floor is now strictly *unnecessary* for threading (kept anyway — modern, already merged; maintainer's call). Always run the exact CI command — `pnpm exec vitest run --coverage --exclude '**/publishedTarball.test.ts'` — locally before pushing a migration, since this failure surfaces **only** under coverage + CI's Node. *Rejected:* duplicate named groups (CI-incompatible, the original plan); positional threading (fragile).
...
4. **Grammar-invariant test** (extends `grammarOrder.test.ts`): every `orderedPatterns` regex (i) **compiles** (a `new RegExp(src, flags)` smoke test on the CI Node; note CI's Node *rejects* duplicate named groups — see decision #2 — so migrated patterns use distinct per-branch names), (ii) carries `g` and `d`, (iii) for migrated patterns, the extractor's read-set ⊆ the pattern's declared group-set.
...
1. **PR 1 — threading substrate + runtime-floor bump (zero *extraction*-behavior change).** Bump `engines.node` to `>=22` and the CI matrix to 22/24/26 (its own changeset — breaking floor change; update the tarball-consumer leg + the coverage job's Node accordingly); add the two `Token` fields; add `d` to all 101 patterns; thread `groups`/`groupSpans` **only for named-group patterns** (none yet → no-op); add the typed-accessor scaffolding (`requireGroup`/`optionalGroup`); add the grammar-invariant test (compiles on Node 22, `g`+`d`). Full suite + corpus unchanged.
...
- **Runtime-floor bump to Node ≥ 22 is breaking (MED).** Dropping Node 18/20 is consumer-facing. Mitigation: an explicit changeset on PR 1 with a clear BREAKING note (pre-1.0, so acceptable in a minor); in return, duplicate named groups keep the patterns clean (no coalescing), and the §7.4 compile smoke-test pins the floor.
...
**Maintainer decision (2026-06-10):** set the runtime floor to Node ≥ 22 (CI matrix 22/24/26), which *permits* duplicate named capturing groups — so the distinct-name/coalesce workaround is dropped in favor of identical cross-branch names (`(?<page>…)` on both branches). This trades a breaking floor bump for cleaner patterns and simpler extractor reads.
