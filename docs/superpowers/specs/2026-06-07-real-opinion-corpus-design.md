...
3. **Scale:** **tiered** — Phase 1 ships a ~1k committed core that gates PRs; Phase 2 (deferred) adds a ~10–50k extended tier on a nightly/manual job. **REVISED at execution (2026-06-08):** the *initial* committed core is **~127 opinions** (3 seeds + 124 real), not ~1k. Real per-opinion cost turned out to be ~10 KB on disk (~4.8 KB gzipped text + ~5.4 KB projection), so ~1k ≈ ~10 MB committed; the ~127 draw proves the net (it surfaced a real crash, #881) at ~1.3 MB and is grown toward ~1k via `corpus:fetch` with a lower STEP. Growing the core to ~1k is deferred per the size/coverage tradeoff.
4. **Sampling:** **stratified** across court level / era / opinion type / OCR-vs-clean, deterministic + reproducible (committed manifest).
5. **Generation model:** ~~Claude-driven via the pgedge-flp MCP~~ ~~direct-DB `pg` script~~ → **REVISED at execution (2026-06-08):** a **CourtListener REST API** fetch script (`corpus:fetch`, needs `COURTLISTENER_API_TOKEN`). Two facts forced this: (a) the MCP returns query results *into the agent context*, and ~1k opinions ≈ 28 KB each ≈ **~7M tokens** of text — far beyond any context; (b) the replica's DB password isn't accessible (only the API token is, via `mise.local.toml`). The fetch script is **self-contained + deterministic over the API**: the selection is the nearest opinion at/after each evenly-spaced id (`?id__gte=N&order_by=id` is the API's `LATERAL` nearest-id — the scheme the MCP validated), filtered to a `[500, 60000]`-char plain_text window, which realizes ~1/4 of the gid count (STEP=22000 → 500 gids → ~124 real opinions), and text streams API→disk (never through the agent). No DB, no `pg` dep, no credential distribution. Replica-free steps (`corpus:regen`, the snapshot test) are unchanged.
