...
Each citation carries a structured `confidence` object with orthogonal axes:

```typescript
const [cite] = extractCitations(text)

cite.confidence.score    // 0.85 — calibrated composite (0..1)
cite.confidence.level    // "high" — categorical bucket
cite.confidence.reasons  // ["known_reporter", "year_plausible", "case_name_present"]
cite.confidence.axes     // { extraction: 0.9, metadata: 0.85, resolution?: 0.95 }
```

**Axes** separate three orthogonal questions:
- `extraction` — P(this is a real citation), calibrated per pattern
- `metadata` — completeness of parsed fields (court, year, case name)
- `resolution` — P(correct antecedent link), short-form citations only

**Levels** provide stable categorical buckets: `"certain"` (≥ 0.95), `"high"` (≥ 0.80), `"medium"` (≥ 0.50), `"low"` (< 0.50). Pin UI thresholds on `level` for stability across calibration updates.

**Reason codes** are machine-readable tags explaining the score: `known_reporter`, `year_plausible`, `case_name_present`, `court_identified`, `reporter_unknown`, `reporter_ambiguous`, `missing_year`, `blank_page`, `exact_antecedent_match`, and more.
...
| Confidence scoring | No | Yes (structured) | score + level + axes + reason codes |
...
pnpm exec vitest run   # Run tests once (3,020 tests, 107 files)
